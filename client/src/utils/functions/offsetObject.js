...
export const handleFile = (file, offset, callback) => {
  ...
  reader.addEventListener(
    ...
    () => {
      ...
      data = reader.result;
      // reads the content file and create an object
  ...
  reader.addEventListener("loadend", () => {
    // create a new Object with the same number of face but with the offsetted object
    const newObjects = createNewObjects(initialObjects, offset);

    // - Recreate the new file
    const newFile = parseExport(newObjects);
    // The new file is ready to be downloaded
    callback(newFile);
  });
...
const parseASCII = (data) => {
  ...
  //createNewObjects(initialObjects);
};
